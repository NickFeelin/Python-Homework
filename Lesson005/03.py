...
def moving(field, move, step):
    ...
    if (3 > x >= 0) and (3 > y >= 0) and result[y][x] == " ":
        ...
        new_point = input("Неправильный ход, повторите ввод: ")
        result = moving(field, new_point, step)
    return result
...
moves_count = 0
is_X_move = True
print('Добро пожаловать в игру "Крестики-нолики". В свой ход вводите координаты, разделенные пробелом.')
while check_win(field) == None and moves_count < 9:
    current_player = "X" if is_X_move else "0"
    field = moving(field, input(
